...
if __name__ == '__main__':
    ...
    tenOne = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(PIL.Image.open('timestep_0.png').convert('RGB'))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
    tenTwo = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(PIL.Image.open('timestep_1.png').convert('RGB'))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
